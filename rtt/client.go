...
func WithHTTPClient(hc *http.Client) Option { return func(c *Client) error { if hc != nil { c.httpClient = hc }; return nil } }

// WithBaseURL overrides the API base. Accepts values with or without trailing slash.
func WithBaseURL(raw string) Option {
    return func(c *Client) error {
        if raw == "" { return errors.New("base url empty") }
...
func New(username, password string, opts ...Option) (*Client, error) {
    ...
    for _, o := range opts { if err := o(c); err != nil { return nil, err } }
    return c, nil
}
...
func (c *Client) doJSON(ctx context.Context, pth string, v any) error {
    ...
    if !strings.HasPrefix(clean, "json/") { clean = path.Join("json", clean) }
