...
func parseHHMM(s string) (time.Time, error) {
    if len(s) != 4 { return time.Time{}, fmt.Errorf("invalid HHmm: %q", s) }
    h := (int(s[0]-'0')*10 + int(s[1]-'0'))
    m := (int(s[2]-'0')*10 + int(s[3]-'0'))
    if h > 23 || m > 59 { return time.Time{}, fmt.Errorf("invalid HHmm: %q", s) }
    return time.Date(0,1,1,h,m,0,0,time.UTC), nil
}
func parseHHMMSS(s string) (time.Time, error) {
    if len(s) != 6 { return time.Time{}, fmt.Errorf("invalid HHmmss: %q", s) }
    h := (int(s[0]-'0')*10 + int(s[1]-'0'))
    m := (int(s[2]-'0')*10 + int(s[3]-'0'))
    sec := (int(s[4]-'0')*10 + int(s[5]-'0'))
    if h > 23 || m > 59 || sec > 59 { return time.Time{}, fmt.Errorf("invalid HHmmss: %q", s) }
...
func (t TimeHHMM) String() string { if t.Time.IsZero() { return "" }; return fmt.Sprintf("%02d%02d", t.Hour(), t.Minute()) }
...
func (t TimeHHMMSS) String() string { if t.Time.IsZero() { return "" }; return fmt.Sprintf("%02d%02d%02d", t.Hour(), t.Minute(), t.Second()) }
