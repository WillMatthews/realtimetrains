...
func (c *Client) SearchStation(ctx context.Context, station string, p SearchParams) (*Container, error) {
    ...
    if p.To != "" { segs = append(segs, "to", p.To) }
    if p.Date != nil {
        y, m, d := p.Date.Date()
        segs = append(segs, fmt.Sprintf("%04d", y), fmt.Sprintf("%02d", m), fmt.Sprintf("%02d", d))
        if p.TimeHHMM != "" { segs = append(segs, p.TimeHHMM) }
    }
    if p.Arrivals { segs = append(segs, "arrivals") }
    path := strings.Join(segs, "/")
    var cont Container
    if err := c.doJSON(ctx, path, &cont); err != nil { return nil, err }
    return &cont, nil
}
